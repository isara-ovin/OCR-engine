...
@app.route('/ocr-label', methods=['GET', 'POST'])
def scan_file():
    if request.method == 'POST':

        image_data = base64.b64decode(request.args.get('label'))
        scanned_text = pytesseract.image_to_string(Image.open(io.BytesIO(image_data)))

        return jsonify({'label': scanned_text})


if __name__ == '__main__':
    # Setup Tesseract executable path
    pytesseract.pytesseract.tesseract_cmd = r'./TesseractOCR/tesseract'
    app.run(debug=True)
